...
static void rotatestack(const Arg *arg) __attribute__((unused));
...
int swallowfloating = 1;
static int attachbelow = 1; // Initialize attachbelow to 0 (false)
static int gappih = 14;     /* inner horizontal gap between windows */
static int gappiv = 14;     /* inner vertical gap between windows */
static int gappoh =
    14; /* outer horizontal gap between windows and screen edge */
static int gappov = 14; /* outer vertical gap between windows and screen edge */
...
pid_t getparentprocess(pid_t p) {
    ...
    if (fscanf(f, "%*u %*s %*c %u", &v) != 1) {
        ...
        fprintf(stderr, "Failed to read from /proc\n");
    }
    fclose(f);
#endif /* __linux__ */
